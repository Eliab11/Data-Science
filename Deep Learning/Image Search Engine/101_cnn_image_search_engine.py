...
model = Model(input_shape = vgg.input, outputs = vgg.layers[- 1].output)
...
pickle.dump(filename_store, open('model/filename_store.p', 'wb'))
pickle.dump(filename_store, open('model/feature_vector_store.p', 'wb'))
...
modelv= load_model('model/vgg16_search_engine.h5', compile = False)

filename_store = pickle.load(open('model/filename_store.p', 'rb'))

feature_vector_store = pickle.load(open('model/feature_vector_store.p', 'rb'))
