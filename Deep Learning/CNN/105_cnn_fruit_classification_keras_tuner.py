...
validation_set = training_generator.flow_from_directory(directory = training_data_dir,
                                                     target_size = (img_width, img_height),
                                                     batch_size = batch_size,
                                                     class_mode = 'categorical')
...
def build_model(hp):
    ...
    model.add(Conv2D(filters= hp.Int("Input_Con_Filters", min_value = 32, max_value = 128, step = 32), kernel_size= (3,3), padding= 'same', input_shape = (img_width, img_height, num_channels)))
    model.add(Activation('relu'))
    model.add(MaxPooling2D())


    for i in range( hp.Int("n_Con_Layers", min_value = 1, max_value = 3, step = 1)):
        
        model.add(Conv2D(filters = hp.Int(f"Input_{i}_Filters", min_value = 32, max_value = 128, step = 32), kernel_size= (3,3), padding= 'same'))
        model.add(Activation('relu'))
        model.add(MaxPooling2D())
...
model_filename = 'model/fruits_cnn_v02.h5'
img_width = 128
img_height = 128
...
image = preprocessing_image(filepath)
make_prediction(image)

# loop through test data

source_dir = 'data/test'
folder_names = ['apple', 'avocado', 'kiwi', 'lemon', 'orange']
actual_labels = []
predicted_labels = []
...
for folder in folder_names:
    ...
    for image in images:
        ...
        processed_image = preprocessing_image(source_dir + '/' + folder + '/' + image)
        prodicted_label, predicted_probability = make_prediction(processed_image)
...
predictions_df['correct'] = np.where(predictions_df['actaul_label'] == predictions_df['predicted_label'], 1, 0)
